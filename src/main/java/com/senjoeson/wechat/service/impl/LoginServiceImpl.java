...
import com.senjoeson.wechat.config.PathConfig;
...
import com.senjoeson.wechat.utils.Config;
import com.senjoeson.wechat.utils.MyHttpClient;
import com.senjoeson.wechat.utils.SleepUtils;
import com.senjoeson.wechat.utils.enums.ResultEnum;
import com.senjoeson.wechat.utils.enums.RetCodeEnum;
import com.senjoeson.wechat.utils.enums.StorageLoginInfoEnum;
import com.senjoeson.wechat.utils.enums.URLEnum;
import com.senjoeson.wechat.utils.enums.parameters.BaseParaEnum;
import com.senjoeson.wechat.utils.enums.parameters.LoginParaEnum;
import com.senjoeson.wechat.utils.enums.parameters.StatusNotifyParaEnum;
import com.senjoeson.wechat.utils.enums.parameters.UUIDParaEnum;
...
public class LoginServiceImpl implements ILoginService {
    ...
    public boolean login() {
        ...
        if (!core.getLoginState()) {
            System.out.println("login进来了吗?");
    ...
    public boolean getQR() {
        String qrPath = PathConfig.getPathPic("login");
    ...
    public void webWxGetContact() {
        ...
        try {
            ...
            for (Iterator<?> iterator = member.iterator(); iterator.hasNext(); ) {
                ...
                System.out.println("获取联系人列表:  " + o.toJSONString());
                //todo
    ...
    public void WebWxBatchGetContact() {
        ...
        try {
            ...
            for (int i = 0; i < contactList.size(); i++) { // 群好友
                JSONObject jsonObject = contactList.getJSONObject(i);
                System.out.println("获取联系人信息: " + jsonObject.toJSONString());
                if (jsonObject.getString("UserName").contains("@@")) { // 群
                    core.getGroupNickNameList().add(jsonObject.getString("NickName")); // 更新群昵称列表
                    core.getGroupList().add(jsonObject); // 更新群信息（所有）列表
                    core.getGroupMemeberMap().put(jsonObject.getString("UserName"),
                            jsonObject.getJSONArray("MemberList")); // 更新群成员Map
                }
            }
        } catch (Exception e) {
            LOG.info(e.getMessage());
